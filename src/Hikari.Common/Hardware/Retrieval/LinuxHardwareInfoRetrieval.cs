...
internal class LinuxHardwareInfoRetrieval : HardwareInfoRetrievalBase, IHardwareInfo
{
    public MemoryStatusInfo GetMemoryStatus()
    {
        MemoryStatusInfo memoryStatus = new();
    ...
    {
        List<BatteryInfo> batteryList = new ();
    ...
    /// 获得cpu信息
    /// </summary>
    /// <param name="includePercentProcessorTime">包括处理器百分比</param>
    /// <returns></returns>
    public List<CpuInfo> GetCpuList(bool includePercentProcessorTime = true)
    {
        List<CpuInfo> cpuList = new ();
    ...
    private void GetCpuUsage(CpuInfo cpu)
    {
    ...
    private UInt64 GetCpuPercentage(string cpuStatLast, string cpuStatNow)
    {
    ...
    {
        List<KeyboardInfo> keyboardList = new();

        KeyboardInfo keyboard = new ();
    ...
    {
        List<MemoryInfo> memoryList = new ();
    ...
    {
        List<MotherboardInfo> motherboardList = new ();

        MotherboardInfo motherboard = new ()
        {
    ...
    {
        List<NetworkAdapterInfo> networkAdapterList = base.GetNetworkAdapterList(includeBytesPersec, includeNetworkAdapterConfiguration);

        if (includeBytesPersec)
        {
            char[] charSeparators = new char[] { ' ' };

            foreach (NetworkAdapterInfo networkAdapter in networkAdapterList)
            {
    ...
    {
        List<PrinterInfo> printerList = new ();
    ...
    {
        List<VideoControllerInfo> videoControllerList = new ();
        ...
        foreach (string line in lines.Where(l => l.Contains("VGA compatible controller")))
        {
            ...
            if (split.Length > 2)
            {
                ...
                if (!string.IsNullOrWhiteSpace(relevant))
                {
                    ...
                    VideoControllerInfo gpu = new () { Description = relevant, Manufacturer = vendor, Name = name };
